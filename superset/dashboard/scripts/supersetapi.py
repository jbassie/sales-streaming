...
class SupersetAPIClient:
    ...
    def create_database(self,database_name, driver, engine, sqlalchemy_uri);
        ...
        r = requests.post(uri = self.base_url + "/database", json=payload, headers=headers_auth)
        r.raise_for_status()

        database_id = r.json()['id']
        return database_id

        def create_database(self, database, schema, table):
            payload = {
        ...
        def create_dashbaord(seld, title):
            payload = {
        ...
        def create_chart(self, dashboard_ids: list, datasource_id:int, datasource_name: str, params: str, slice_name:str, viz_type:str ):
            ...
            r = requests.post(url=base_url + "/chart", json=payload, headers= headers_auth)
            r.raise_for_status()
